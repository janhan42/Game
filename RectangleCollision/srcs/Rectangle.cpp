...
void Rectangle::update(float screenWidth, float screenHeight, float deltaTime, bool inPut[4], std::vector<Wall>& walls)
{
	if (inPut[0]) { // W key
		accelation.y = -5.0f; // Move up
	} else if (inPut[1]) { // S key
		accelation.y = 5.0f; // Move down
	...
	if (inPut[2]) { // A key
		accelation.x = -5.0f; // Move left
	} else if (inPut[3]) { // D key
		accelation.x = 5.0f; // Move right
	} else {
		if (Velocity.x != 0){
			if (Velocity.x > 0)
				accelation.x = -2.5f;
	...
	// Velocity.y += gracity.y * deltaTime;
